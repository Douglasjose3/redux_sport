...
type CarrinhoState = {
  //lembrando que esse é o estado
  itens: Produto[]; //tipagem dos itens
};

const initialState: CarrinhoState = {
  //precisa criar um objeto para o estado inicial
...
const carrinhoSlice = createSlice({
  ...
  reducers: {
    //cria toda mudança do estado
    adicionar: (state, action: PayloadAction<Produto>) => {
      //esse estado busca o initialState //<aqui tem que passar o vai estar no PayloadAction>
      const produto = action.payload; //armazena o PayloadAction<Produto> //objeto do produto

      if (state.itens.find((p) => p.id === produto.id)) {
        //precisa tipar os itens
